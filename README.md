...
`user1@sentinellab.local` initially authenticates successfully from Hyderabad. Shortly afterward, four failed authentication attempts are recorded from `185.220.101.10`. The same source then successfully authenticates as the user from New York.

Six minutes later, suspicious PowerShell activity appears on `DESKTOP-LAB01`. The PowerShell process is launched by `winword.exe` and contains `-ExecutionPolicy Bypass` and `-EncodedCommand`.

The investigation examines whether the authentication and endpoint events form a consistent sequence that could indicate account compromise.

---

## Lab Objectives

- Analyze the authentication history of a potentially compromised account.
- Identify repeated failed authentication attempts from a common source.
- Determine whether a suspicious source later achieved successful authentication.
- Examine changes in authentication location and source IP.
- Review endpoint activity associated with the account.
- Correlate identity and endpoint events using user, host, and time.
- Investigate suspicious PowerShell execution following authentication.
- Construct a chronological incident timeline.
- Separate confirmed observations from plausible relationships and unknowns.
- Evaluate alternative explanations before assigning a verdict.
...
Account compromise investigations should connect identity and endpoint evidence into a single timeline.

In this case, the useful chain is:

**Failed authentication → successful authentication → short time gap → suspicious endpoint activity**

The strongest investigation does not stop at identifying a suspicious login. It asks what happened **after authentication** and whether the subsequent activity supports or weakens the compromise hypothesis.
...
- Identify a source that later authenticated successfully.
- Review authentication location and IP changes.
...
- Identify telemetry required for confirmation.

---

## Conclusion

The investigation identified a suspicious sequence involving repeated failed authentication attempts from `185.220.101.10`, followed by a successful authentication for `user1` from New York. Six minutes later, suspicious PowerShell activity was observed on `DESKTOP-LAB01`, launched by `winword.exe` with `-ExecutionPolicy Bypass` and `-EncodedCommand`.

The combined evidence supports a **possible account compromise** assessment, but the available synthetic telemetry is insufficient to confirm unauthorized access or compromise. Additional identity and endpoint telemetry would be required for confirmation.
